...
export default function BuchungenDetailPage() {
  ...
  const [kursplanList, setKursplanList] = useState<Kursplan[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, kursplanData] = await Promise.all([
        LivingAppsService.getBuchungen(),
        LivingAppsService.getKursplan(),
      ]);
      setKursplanList(kursplanData);
  ...
  function getKursplanDisplayName(url?: unknown) {
    if (!url) return '—';
    const refId = extractRecordId(url);
    return kursplanList.find(r => r.record_id === refId)?.fields.uhrzeit ?? '—';
  }
  ...
  return (
    ...
    >
      <RecordHeader title={record.fields.vorname ?? 'Buchungen'} />
      ...
      <RecordSection title="Details" cols={2}>
        ...
        <RecordField label="Kurs" value={getKursplanDisplayName(record.fields.kurs)} format="text" />
        <RecordField label="Kursdatum" value={record.fields.kursdatum} format="date" />
        <RecordField label="Status" value={record.fields.buchungsstatus} format="pill" />
      </RecordSection>
