...
export default function BuchungenPage() {
  ...
  function getKursplanDisplayName(url?: unknown) {
    ...
    return kursplanList.find(r => r.record_id === id)?.fields.uhrzeit ?? '—';
  }
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/buchungen/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getKursplanDisplayName(record.fields.kurs)}</span></TableCell>
