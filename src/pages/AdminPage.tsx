...
import { KursplanViewDialog } from '@/components/dialogs/KursplanViewDialog';
...
const KURSPLAN_FIELDS = [
  { key: 'kursname', label: 'Kursname', type: 'lookup/select', options: [{ key: 'vinyasa_flow', label: 'Vinyasa Flow' }, { key: 'yin_yoga', label: 'Yin Yoga' }, { key: 'hatha_yoga', label: 'Hatha Yoga' }, { key: 'power_yoga', label: 'Power Yoga' }, { key: 'restorative_yoga', label: 'Restorative Yoga' }, { key: 'meditation', label: 'Meditation' }, { key: 'pranayama', label: 'Pranayama' }] },
  { key: 'trainer', label: 'Trainer/in', type: 'lookup/select', options: [{ key: 'betreiberin', label: 'Ich (Studiobetreiberin)' }, { key: 'trainer_1', label: 'Trainer/in 1' }, { key: 'trainer_2', label: 'Trainer/in 2' }, { key: 'trainer_3', label: 'Trainer/in 3' }] },
  ...
  { key: 'max_plaetze', label: 'Maximale Plätze', type: 'number' },
  { key: 'kurs_status', label: 'Kursstatus', type: 'lookup/radio', options: [{ key: 'findet_statt', label: 'Findet statt' }, { key: 'abgesagt', label: 'Abgesagt' }] },
];
const BUCHUNGEN_FIELDS = [
  ...
  { key: 'kurs', label: 'Kurs', type: 'applookup/select', targetEntity: 'kursplan', targetAppId: 'KURSPLAN', displayField: 'uhrzeit' },
  { key: 'kursdatum', label: 'Kursdatum', type: 'date/date' },
  { key: 'buchungsstatus', label: 'Status', type: 'lookup/radio', options: [{ key: 'gebucht', label: 'Gebucht' }, { key: 'warteliste', label: 'Warteliste' }, { key: 'storniert', label: 'Storniert' }] },
];

const ENTITY_TABS = [
  { key: 'kursplan', label: 'Kursplan', pascal: 'Kursplan' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('kursplan');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kursplan': return (data as any).kursplan as Kursplan[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'buchungen':
        lists.kursplanList = (data as any).kursplan ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'buchungen' && fieldKey === 'kurs') {
      const match = (lists.kursplanList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.uhrzeit ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kursplan': return KURSPLAN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kursplan': return {
        create: (fields: any) => LivingAppsService.createKursplanEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKursplanEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKursplanEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kursplan' || dialogState?.entity === 'kursplan') && (
        <KursplanDialog
          open={createEntity === 'kursplan' || dialogState?.entity === 'kursplan'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kursplan' ? handleUpdate : (fields: any) => handleCreate('kursplan', fields)}
          defaultValues={dialogState?.entity === 'kursplan' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'buchungen' || dialogState?.entity === 'buchungen') && (
        <BuchungenDialog
          ...
          kursplanList={(data as any).kursplan ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Buchungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Buchungen']}
        />
      )}
      {viewState?.entity === 'kursplan' && (
        <KursplanViewDialog
          open={viewState?.entity === 'kursplan'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kursplan', record: r }); }}
        />
      )}
