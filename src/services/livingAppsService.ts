...
import type { Kursplan, Buchungen, CreateKursplan, CreateBuchungen } from '@/types/app';
...
export class LivingAppsService {
  // --- KURSPLAN ---
  static async getKursplan(): Promise<Kursplan[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSPLAN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Kursplan[];
    return enrichLookupFields(records, 'kursplan');
  }
  static async getKursplanEntry(id: string): Promise<Kursplan | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KURSPLAN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Kursplan;
    return enrichLookupFields([record], 'kursplan')[0];
  }
  static async createKursplanEntry(fields: CreateKursplan) {
    return callApi('POST', `/apps/${APP_IDS.KURSPLAN}/records`, { fields: cleanFieldsForApi(fields as any, 'kursplan') });
  }
  static async updateKursplanEntry(id: string, fields: Partial<CreateKursplan>) {
    return callApi('PATCH', `/apps/${APP_IDS.KURSPLAN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kursplan') });
  }
  static async deleteKursplanEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KURSPLAN}/records/${id}`);
  }
